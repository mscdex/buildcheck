...
function getKind(prop) {
  ...
  if (isWindows) {
    ...
    const result = spawnSync(
      this[prop],
      [ '-EP', `-T${prop === '_cc' ? 'c' : 'p'}`, this._tmpInFile],
      spawnOpts
    );
...
class BuildEnvironment {
  constructor(cfg) {
    ...
    if (isWindows) {
      ...
      if (versions.length === 0)
        throw new Error('Unable to detect compiler type');
      const vs = versions.shift(); // Use newest for now
      cc = vs.cl;
      cxx = cc;
      this._includePaths = vs.includePaths;
      this._libPaths = vs.libPaths;
  ...
  checkDeclared(type, symbolName, opts) {
    ...
    const headersList = renderHeaders(getDefaultHeaders(this, type));
...
main ()
{
  ...
  return 0;
    ...
    this._debug(`>>> Checking if '${symbolName}' is declared... ${result}`);
    if (result !== true) {
